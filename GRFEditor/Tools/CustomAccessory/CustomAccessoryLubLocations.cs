...
namespace GRFEditor.Tools.CustomAccessory {
	public sealed class CustomAccessoryLubLocations {
		...
		public void SetManualEditPaths(string accessoryIdPath, string accnamePath) {
			if (!string.IsNullOrWhiteSpace(accessoryIdPath))
				EditAccessoryIdPath = accessoryIdPath;

			if (!string.IsNullOrWhiteSpace(accnamePath))
				EditAccnamePath = accnamePath;
		}
		...
		public static CustomAccessoryLubLocations Resolve(GrfHolder grf = null) {
			...
			var accessoryIdDisk = ResolveDiskOnlyPath(
				ActiveProjectProfile.GetConfiguredAccessoryIdPath(),
				AccessoryIdDiskRelativeCandidates);
			var accnameDisk = ResolveDiskOnlyPath(
				ActiveProjectProfile.GetConfiguredAccNamePath(),
		...
		private static IEnumerable<string> GetSearchRoots() {
			...
			string clientFolder = ActiveProjectProfile.GetClientFolderPath();
			if (!String.IsNullOrEmpty(clientFolder) && Directory.Exists(clientFolder)) {
				string fromClient = add(clientFolder);
				if (fromClient != null)
					yield return fromClient;
			}
